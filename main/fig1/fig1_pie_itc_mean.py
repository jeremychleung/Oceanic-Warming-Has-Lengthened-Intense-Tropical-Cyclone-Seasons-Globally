...
ibt_year = pd.read_csv(r'..\DataSet\ibtracs_itc_p5_p95.csv', index_col=0, keep_default_na=False, na_values='')
...
for iob,ob in enumerate(oceanbasin):
    ...
    wd = x1 - x0
    ax.bar(x0,0.3, bottom=0.4, width=wd, edgecolor='w', color='b', linewidth=1, align="edge")
    ...
    x1 = np.argwhere(cond1)[0][0]/365 * 2*np.pi
    wd = x1 - x0
    ax.bar(x0,0.3, bottom=0.7, width=wd, edgecolor='w', color='r', linewidth=1, align="edge")
    ...
    fig.savefig('pie_itc.'+ob.lower()+'.png', dpi=600)
    plt.show()
    plt.close()