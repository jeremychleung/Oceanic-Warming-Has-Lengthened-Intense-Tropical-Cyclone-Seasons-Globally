...
u850 = np.load('./data/u850.npy', allow_pickle=True)
v850 = np.load('./data/v850.npy', allow_pickle=True)

lats = v850.item()['lat']
lons = v850.item()['lon']
years = v850.item()['year']
...
for iob,ob in enumerate(oceanbasin):
    u1 = u850.item()[ob.lower()+'1']
    v1 = v850.item()[ob.lower()+'1']
    u2 = u200.item()[ob.lower()+'1']
    v2 = v200.item()[ob.lower()+'1']
    u1 = np.array(u1)
    v1 = np.array(v1)
    u2 = np.array(u2)
    v2 = np.array(v2)

    vws = np.sqrt((u1 - u2)**2 + (v1 - v2)**2)
    vwsm = np.nanmean(vws, axis=0)
    ...
    cs = m.contour(mX,mY,vwsm, levels=np.arange(0,100,5), linewidths=0.8)
    ...
    fig.savefig('vws_reg_near.'+f'{ob.lower()}1'+'.png', dpi=600)
    plt.show()
...
cb.ax.set_xlabel(r'Trend [m s$^{-1}$/dec]', fontsize=12, labelpad=1)

fig.savefig('cb.png', dpi=600)