...
df = pd.read_csv('bempi_area_wgt_term_trend.csv', index_col=0, keep_default_na=False)
...
for pp in range(2):
    ...
    for iob,ob in enumerate(oceanbasin):
        ...
        term1 = sr['ksk_trend']*10
        term2 = sr['rat_trend']*10

        x = np.array([0.7,0.9,1.1,1.3]) + iob
        y = np.array([term1, term2, right, left])

        bb = ax.bar(x,y, width=0.15, color=bcolors, edgecolor='k', linewidth=0.4)
        print(y)

    ax.legend(bb, [r'$\overline{RAT}\times{dKSK/dt}$',
                   r'$\overline{KSK}\times{dRAT/dt}$',
                   r'$\overline{RAT}\times{dKSK/dt}+\overline{KSK}\times\Delta{RAT}/\Delta{t}$',
                   r'$\Delta{MPI}/\Delta{t}$'], loc='upper left', prop={'size':9})
    ...
    fig.savefig('bempi_area_wgt_term_trend.'+f'{pp:1d}'+'.png', dpi=600)
    plt.show()