...
#%%
mpidata = np.load('../mpi_data.npy', allow_pickle=True)
...
for var in varlist:
    ...
    for iob,ob in enumerate(oceanbasin):
        ...
        for pp in range(2):
            ...
            #-------------------------------------------------
            if var != 'ksk':
                data = mpidata.item()[ob.lower()+f'{pp:1d}'][var]
            else:
                xmpi = mpidata.item()[ob.lower()+f'{pp:1d}']['vmax']
                xrat = mpidata.item()[ob.lower()+f'{pp:1d}']['rat']
                xfac = mpidata.item()[ob.lower()+f'{pp:1d}']['fac']

                xrat = xrat - 1  #different RAT expression, Ts/To to (Ts-To)/To
                CKCD = 0.9
                VREDUC = 0.8
                data = xmpi**2/(VREDUC**2 * CKCD * xrat)
